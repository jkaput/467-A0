...
callback_func (GtkWidget *widget, GdkEventKey *event, gpointer callback_data)
{
    ...
    switch (event->keyval) {
        ...
        case GDK_KEY_s: {
	  image_source_data_t *frm = calloc(1, sizeof(image_source_data_t));;
	  isrc->get_frame(isrc, frm);
	  image_u32_t *img = image_convert_u32(frm);
	  image_u32_write_pnm(img, "src/imagesource/image.ppm");
	  isrc->release_frame(isrc, frm);
	  printf("s key pressed, capturing image\n");
	  free(frm);
	  break;
        }
...
main (int argc, char *argv[] )
{
    ...
    //isrc->set_feature_value(isrc, 17, 0);
    //isrc->set_feature_value(isrc, 18, 0);
    //isrc->set_feature_value(isrc, 20, 0);
